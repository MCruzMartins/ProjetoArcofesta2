  ...
  <?php
	...
	echo "senha".password_hash(123, PASSWORD_DEFAULT);
		...
		if(!empty($dados["btnlogin"])){
			...
			$sql = "SELECT idcolaborador,nome, email, senha 
                        FROM funcionario 
                        WHERE email =:usuario  
                        LIMIT 1";
			...
			if(($resultado) AND ($resultado->rowCount() != 0)){
				...
				if(password_verify($dados['senha'], $linha['senha'])){
					...
					$_SESSION['idcolaborador']= $linha['idcolaborador'];
		...
		<div class="card">
			...
			<div class="card-body">
				<form method="POST" action="">
					...
					<a href="frmfuncionario.php"><button class="btn float-right">Cadastre-se</a></button>
					</div>
